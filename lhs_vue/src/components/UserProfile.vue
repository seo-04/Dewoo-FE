  ...
  <div class="main-container">
      ...
      <div v-if="activeTab === 'account'" class="tab-content active" id="account-tab">
        ...
        <div class="info-group">
          ...
          <div class="info-value-wrapper">
            ...
            <button class="change-btn"><img src="@/assets/Edit.svg" alt="Edit" class="edit-icon">Change</button>
          </div>
        </div>
        <div class="info-group">
    ...
    <div v-if="isModalOpen" class="modal-overlay show" @click.self="closeModal">
      <div class="modal-content">
        <p style="text-align: left; font-size: 40px; font-weight: bold">카드 추가</p>
        ...
        <div class="card_input">
          ...
          <label for="CVC" style="left: 280px">CVC</label>
...
// 원본 데이터가 바뀔 때마다 자동으로 다시 계산됩니다.

const filteredBookings = computed(() => {
  const today = new Date();
  // 현재 날짜를 YYYY-MM-DD 형식으로 변환 (시간은 무시)
...
function addCard() {
  ...
  cards.value.unshift(cardToAdd); // 배열 맨 앞에 새 카드 추가
...
// --- 4. 감시자 (Watchers) ---
// 특정 데이터의 변경을 감지하여 복잡한 로직(입력 포매팅 등)을 수행합니다.
...
// 컴포넌트가 생성되거나 사라질 때 특정 로직을 수행합니다.
...
onBeforeUnmount(() => {
  ...
  <style scoped>
    ...
    .card_input {
      display: flex;
      position: relative;
      margin-bottom: 25px; /* 다음 입력 필드와의 간격 */
      width: 512px;
      height: 56px;
    }
    ...
    .card_input label {
      position: absolute;
      top: -10px; /* input 테두리 위로 올림 */
      left: 10px;
      background-color: white; /* 글자 배경을 흰색으로 하여 input 테두리를 덮음 */
      padding: 0 5px;
      font-size: 14px;
      color: black;
      z-index: 1; /* label이 input보다 위에 오도록 */
    }
    ...
    .card_input input[type="checkbox"]:checked{
      accent-color: black;
    }
